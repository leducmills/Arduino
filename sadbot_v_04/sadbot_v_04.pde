...
// Adjust Photocell tolerance for various times of day
...
Stepper left_motor(STEPS, 11, 12); // left motor goes to digital pins 11 (direction) & 12 (step)
...
int lowest;
// to store highest photocell value
int highest;
...
void loop() {
  ...
  photo_up = analogRead(2);
  //Serial.print("up");
  Serial.println(photo_up);
  
  photo_down = analogRead(3);
  //Serial.print("down");
  Serial.println(photo_down);
  
  photo_left = analogRead(0);
  //Serial.print("left");
  Serial.println(photo_left);
  
  photo_right = analogRead(1);
  //Serial.print("right");
  ...
  if ((lowest + 200) < highest ) {  //if lowest value indicates a covered photocell, draw towards lowest
